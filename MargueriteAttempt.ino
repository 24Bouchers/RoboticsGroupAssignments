...
void fronterror (float frontScan){
    if (frontScan > 60)//limiting the input
    {frontScan = 60;}//if
...
float pidcalc(float currentReading) {
    ...
    if (currentReading > 30)//limiting the input
    {currentReading = 30;}//if

  const double desiredState = 30; 

  const double kp = 0.9; // Adjust kp for quicker response
  const double ki = 0.1; // Reduce ki for less integration
  const double kd = 1.2; // Increase kd for faster damping
