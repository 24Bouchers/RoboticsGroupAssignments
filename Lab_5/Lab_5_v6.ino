...
const float WHEEL_DIS = 8.2;  //possibly 8.5
float Sl = 0.0F;
float Sr = 0.0F;
float pidResult = 0;
double kiTotal = 0.0;
...
const double KP1 = -60.0;  // Adjust kp for quicker response, 30 works
...
const unsigned long SCAN_PERIOD = 500;
...
void calculatepos() {
  ...
  if (currentMillis - calc_prevMillis >= CALC_PERIOD && encoder_done) {
    ...
    //CHANGED VALUE OF CARE FOR TESTING
    ...
    if (measured_distance_left < important_left && measured_distance_left > 0) {
      ...
      proportional_left = KPleft * error_angle;
    }  //
    else {
    ...
    if (measured_distance_far_left < important_left && measured_distance_far_left > 0) {
      ...
      proportional_far_left = KP_far_left * error_angle;
    }  //
    else {
...
void Move() {
  ...
  if (currentMillis - move_prevMillis >= MOVE_PERIOD && check_done) {
    ...
    if ((leftSpeed > MAX_SPEED) || (rightSpeed > MAX_SPEED)) {
